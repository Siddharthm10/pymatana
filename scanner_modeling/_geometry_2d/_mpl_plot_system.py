...
from ._utils import fov_corners_vertices_2d
# import numpy as np
...
def plot_scanner_from_vertices_2d_mpl(
    ...
    plate_polycoll = plot_polygons_from_vertices_mpl(
        plate_polygon_tensor, ax, color="C0", alpha=0.5
    )
    crystal_polycoll = plot_polygons_from_vertices_mpl(
        crystal_polygon_tensor, ax, color="C1", alpha=0.5
    )
    fov_polycoll = plot_fov_as_rectangle_mpl(
        fov_dict, ax, fc="none", ec="C2", alpha=0.5
    )
    ax.autoscale()
    return {
